...
import {
  getAllArrangementsForSong,
  getAllAttachmentsForArrangement,
  getAllSongs,
} from "./helpers/pcoApiWrapper";
  ...
  async (req) => {
    try {
      ...
      for (let i = 0; i < arrangements.length; i++) {
        const attachments = await getAllAttachmentsForArrangement(
          song.id,
          arrangements[i].id,
        );
  ...
  async (event: ScheduledEvent) => {
    ...
    try {
      ...
      const queue = getFunctions().taskQueue(
        "projects/worship-repository/locations/europe-west3/functions/processSongTask",
      );
      const enqueues = songs.map(
        (song) => queue.enqueue(song), // This adds the work to the queue
      );
